...
hashes. Moreover, these types are opaque in the sense that we do not
expose its constructor. They are instances of the `IsString` and `Show`
...
All cryptographic hashes are instance of the type class `Hash` and as
the generic function `hashFile :: Hash h => FilePath -> h` can be used
to compute the hash of a file generic function. The top level
signature is necessary in this case so that ghc can deduce which hash
to compute.
...
Notice that for pure values like ByteString, we use function `hash` to
compute its digest. The result is also a pure value.

# Design philosopy.

The raaz package will expose cryptographic primitives whcih will be
...
:   In literature cryptographic data like hashes are often treated just
    ...
    just 32-byte strings. For a crypto library this is bad design. This
...
:   We have an interface to secure memory, i.e. memory that is locked and will
    ...
    internal state. See the classes `Memory` in the library.
